...
The following example demonstrates the `LRAPA` workflow using the package’s example data for human embryonic stem cells (hESC, D0) and hESC-derived neurons (D7) samples. Each step is presented as a 
separate R code chunk, which can be combined into a script (`scripts/APA_analysis.R`) for pipeline execution.
...
- **Custom Data**: Use your own IsoQuant output files by updating paths. Ensure that the `OUT.corrected_reads.bed` and `OUT.read_assignments.tsv` files are found in the provided paths for each 
sample:
